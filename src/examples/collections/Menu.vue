<template>
    <div>
        <List :items="items" :child="MenuButton" :isFocused="isFocused" id="menu" v-on:onFocusChange="onFocusChange"/>
    </div>
</template>
...
import {
  ...
  unRegisterFocusDispatcher,
    ...
    onFocusChange({ item }) {
      ...
      console.error(this.$router.currentRoute.hash.split('#/').pop() , item.path)  
